...
class ProviderController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ( $model->load(Yii::$app->request->post())) {
            //$model->ProviderIdProof = implode("," , $model->_ProviderIdProof);
            if($model->save()){
    ...
    public function actionProviderreg()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            ...
            return $this->render('providerRegistration', [
                'model' => $model,
            ]);
        }
    }

    public function actionFindprovider(){
        $searchModel = new ProviderSearch();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);

        return $this->render('findProvider', [
            'searchModel' => $searchModel,
            'dataProvider' => $dataProvider,
        ]);
    }

    public function actionFastprovider(){
        $model = new PROVIDER();
        if($model->load(Yii::$app->request->post()) & $model->save()){
            return $this->redirect(['view', 'id' => $model->PROVIDER_ID]);
        }else{
            return $this->render('fastRegistrationProvider', [
                'model' => $model,
            ]);
        }
    }   
